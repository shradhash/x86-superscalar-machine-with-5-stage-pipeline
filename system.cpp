...
static __inline__ u_int64_t cs505_be64toh(u_int64_t __x) { return (((u_int64_t)be32toh(__x & (u_int64_t)0xFFFFFFFFULL)) << 32) | ((u_int64_t)be32toh((__x & (u_int64_t)0xFFFFFFFF00000000ULL) >> 32)); }
...
void System::dram_read_complete(unsigned id, uint64_t address, uint64_t clock_cycle) {
	...
	for(int i=0; i<64; i+=8) {
		...
		tx_queue.push_back(make_pair(cs505_be64toh(*((uint64_t*)(&ram[((address&(~63))+((address+i)&63))]))),tag->second));
	}
...
void System::tick(int clk) {
	...
	if (main_time % (ps_per_clock * 1000) == 0) {
		...
		if (ch != ERR) {
			if (!(interrupts & (1<<IRQ_KBD))) {
				...
				tx_queue.push_back(make_pair((int)IRQ_KBD,(int)IRQ));
				keys.push(ch);
			}
		}
	}
	...
	if (top->reqcyc) {
		...
		if (rx_count) {
			switch(cmd) {
			case MEMORY:
				*((uint64_t*)(&ram[((xfer_addr&(~63))+((xfer_addr + ((8-rx_count)*8))&63))])) = cs505_be64toh(top->req);	// critical word first
				break;
			case MMIO:
				...
				if (show_console)
					if ((xfer_addr - 0xb8000) < 80*25*2) {
						...
						for(int shift = 0; shift < 8; shift += 2) {
							int val = (cs505_be64toh(top->req) >> (8*shift)) & 0xffff;
...
long long syscall_cs505(long long rax, long long rdi, long long rsi, long long rdx, long long r10, long long r8, long long r9)
{
	switch (rax) {
	case __NR_exit:
